...
void PController(sensor_color_t & Color1, BrickPi3 & BP, CalculatingErrorData & data_struct) {
    float tp = 30;                                           // Constant value to determine maximum motor dps
    float kp = 0.3;
    float kd = 3.0;                                       // Constant value to determine the sharpness of the turns the robot takes
    ...
    while (true) {
        ...
        integral += error;
        int turn = kp * error + kd * derivative + ki * integral;
	cout << "INTEGRAL: " << integral << endl;                              // Convert error value to turn value
        int speedA = tp + 1.1* turn;                             // Set new speed of motor A
        int speedD = tp - 1.1* turn;                             // Set new speed of motor B
